...
- **(L-1) Log Recording**
  ...
  - **MUST NOT**: Do not log or print sensitive information (e.g., passwords, tokens, API keys).

## 7. Secrets & Credential Handling
- **MUST NOT**: Never include real credentials in code, logs, comments, or responses. Use placeholders such as `YOUR_API_KEY` in all examples.
- **MUST NOT**: Frontend code MUST NOT contain private API keys. Sensitive logic and API calls MUST be handled on the backend.
- **MUST**: Ensure `.env` and secret files are always included in `.gitignore`.
- **MUST**: If a secret is detected in code or output, redact it immediately.
- **MUST**: Refuse to proceed if secure handling of secrets cannot be ensured.
