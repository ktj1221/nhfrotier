...
| 항목 | 결정 | 상태 |
|---|---|---|
| 사용자 인증 | 사내 SSO 연계, **Backend에서 검증** | 방식 미확정 |
| 인가 | 프로젝트 멤버십 기반 접근 제어 | 권한 모델 미확정 |
...
| 조항 | 요구사항 | 대응 |
|---|---|---|
| A-1 | 사용자 개별 식별, 공유 계정 금지 | **미적용** — 프로토타입은 `UserContext`로 사용자를 선택만 한다. 운영은 SSO로 대체 |
...
| A-3 | 최소권한 원칙, 권한 변경 로깅 | **미적용** — 프로젝트 권한 모델 확정 후 `history_events` / `audit_logs`에 기록 |
...
| D-2 | 파라미터 바인딩 쿼리 | **적용** — 프로토타입은 better-sqlite3 prepared statement 사용. 운영은 JPA/파라미터 바인딩 |
...
| L-1 | [시각, 사용자 ID, 출발지 IP, 요청/행위, 성공/실패] 포함 | **미적용** — 로그 표준 포맷 정의 필요 |
...
| **A03 인젝션 (SQLi)** | 프로젝트·댓글·검색 조회 | 파라미터 바인딩만 사용. 동적 문자열 연결 금지 |
