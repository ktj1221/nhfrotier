...
IA의 `D3`. NH가 보유한 디자인 자산을 **자산 종류별**(로고·워드마크 / 컴포넌트 / 화면 템플릿 / 컬러·타이포 토큰)로 한 화면에 펼쳐 보여준다. 각 자산은 출처(올원뱅크 / 기업인터넷뱅킹 / NH공통)와 **실물 보유 여부**를 배지로 표시하고, 자산을 프로젝트에 담아 AI 생성 컨텍스트로 넘기는 진입점을 제공한다.
